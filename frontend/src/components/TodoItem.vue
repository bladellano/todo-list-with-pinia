<template>
  ...
  >
    <div class="flex items-start space-x-2 md:space-x-3">
      ...
      <!-- Ações -->
      <div class="flex flex-col space-y-0.5 md:space-y-1 flex-shrink-0">
        <!-- Botão Done/Undone -->
        <button
          @click="$emit('toggle-done', todo)"
          class="p-1.5 md:p-2 rounded transition"
          :class="todo.done 
            ? 'text-green-600 hover:bg-green-50' 
            : 'text-gray-400 hover:bg-gray-50'"
          :title="todo.done ? 'Marcar como pendente' : 'Marcar como concluída'"
        >
          <svg v-if="todo.done" class="w-4 h-4 md:w-5 md:h-5" fill="currentColor" viewBox="0 0 24 24">
